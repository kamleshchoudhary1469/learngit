...
router.post('/generate-token/ssologin',
            ...
            if (status == 200 && data && data?.sAMAccountName) {
                ...
                if (ssostatus == 200 && ssodata) {
                    ...
                    const sdata = `{
                            ...
                            "a_process_seqno": "",
                            "a_iud_type": "${!x?.[service] ? "emp_code" : "login_id"}",
                    ...
                    <script>
                        window.nightManager=()=>{
                            return ${JSON.stringify({ ...authdata, profileImg: ssodata?.jpegPhoto, servicepage: service, userdetails })}
                        }
...
export { router as ssoLoginRouter };