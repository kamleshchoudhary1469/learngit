...
// FMS Salary Annex2 Decl Insert Details (Insert/Update/Delete) – ITMS_TIUD_005

router.post('/salary-annex2-decl',
    validationRequest,
    [currentUser, requireAuth],
    validateSqlInjection,
    async (req: Request, res: Response) => {
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi && req?.currentUser?.itmsapi?.salary_annex2_decl) {
                ...
                const procType = 'get_tiud_ifms_salary_annex2_decl';
...
export { router as salaryAnnex2ADECLRouter };