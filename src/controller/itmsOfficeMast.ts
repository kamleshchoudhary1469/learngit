...
// ITMS Office Mast Insert Details (Insert/Update/Delete) – ITMS_MIUD_001
// rowid_seq, office_no should be change every time we want to hit 

router.post('/office-mast',
   validationRequest,
   [currentUser, requireAuth],
   validateSqlInjection,
   async (req: Request, res: Response) => {
      if (req?.currentUser !== undefined) {
         if (req?.currentUser?.itmsapi && req?.currentUser?.itmsapi?.office_mast) {
            const { iud_seqno, a_iud_type, rowid_seq, office_no, treasury_no, ddo_no, gstn_no, ain_no, tan_no, office_name, office_address, city, state, pin_code, office_phone, status } = req.body;
            const procType = 'get_miud_itms_office_mast';
            ...
            var sdata = sDataWriter(req, iud_seqno, `
               ...
               "itms_office_mast": {
                     ...
                     "gstn_no":"${gstn_no ? gstn_no : ""}", 
                     "ain_no":"${ain_no ? ain_no : ""}", 
                     "tan_no":"${tan_no ? tan_no : ""}",
                     "office_name":"${office_name ? office_name : ""}", 
                     "office_address":"${office_address ? office_address : ""}", 
                     "city":"${city ? city : ""}",
                     "state":"${state ? state : ""}", 
...
export { router as officeMastRouter };