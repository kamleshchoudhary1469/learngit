...
router.post('/get-req-tran',
    validationRequest,
    validateRequestBodyKeys(["requestid", "emp_code", "header_req"]),
    ...
    async (req: Request, res: Response) => {
        ...
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi) {
                ...
                const { requestid, emp_code, acc_year, header_req } = req.body;
                const procType = 'get_gtdb_req_tran';
                ...
                var sdata = `{
                        ...
                        "a_page_to":"1",
                        "a_user_level":"1",
                        "a_login_type":"emp_code",
                        "a_filter_clause":[]
                    }`

                const result: any = await execute(`BEGIN PKG_TAXCPC_APP.PROC_CALL_MAIN_INOUT(session_seqno_replace, iud_seqno_replace,:a_in_parameter_json,:a_out_parameter_json,'${emp_code}');END;`, sdata, procType, req);
                ...
                return res.status(ok).send({
                    status: "SUCCESS", errdata: errors ? errors : null, [procType]: {
                        detail,
                        header
                    }, requestid, message: success_msg ? success_msg : error_message
                });
...
export { router as reqTranDetailsRouter };