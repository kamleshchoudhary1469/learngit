...
router.post('/get-sal-annex-breakup',
    validationRequest,
    validateRequestBodyKeys(["requestid", "emp_code", "acc_year", "header_req", "bill_no"]),
    ...
    async (req: Request, res: Response) => {
        ...
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi) {
                ...
                const { bill_no, emp_code, header_req, requestid, acc_year } = req.body;

                const procType = 'get_gtdt_salary_tran_load_proj';
                ...
                var sdata = `{
                    "a_session_seqno": "session_seqno_replace",
                        ...
                        "a_in_parameter_json": [{
                        "a_rowid_seq": "", 
                        "a_deductee_panno": "${bill_no ? bill_no.toUpperCase() : ""}"
                        }]
                    }`
...
export { router as salAnnexBreakupRouter };