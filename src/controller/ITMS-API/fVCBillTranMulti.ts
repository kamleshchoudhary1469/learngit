    ...
    async (req: Request, res: Response) => {
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi && req?.currentUser?.itmsapi?.fvc_bill_tran) {
                ...
                const { iud_seqno, a_iud_type, requestid, source, request_type, rowid_seq, bill_type, object_head_code, budget_head_code, office_no, ddo_no, treasury_no, bill_number, bill_date, pay_year, pay_month,
                    vendor_details 
                } = req.body;

                // bill_id,
                //vendor_code, vendor_name, vendor_panno, place_of_supply, vendor_gstn_no, contract_value, gross_inv_amt, basic_amt, tax_amt, gst_tds_amt, income_tax_tds_amt, if_exempted, exempted_reason,

                const procType = 'get_tiud_ifms_fvc_bill_tran_multi';
                ...
                var sdata = sDataWriter(req, iud_seqno, `
                    ...
                    "ifms_fvc_bill_tran": {
                        ...
                        "pay_month": "${pay_month ?? ""}",
                        "vendor_details": ${JSON.stringify(vendor_details)}
                    }
                `);
                ...
                console.log(sdata, 'sdata of fvc-bill-tran-multi')
                ...
                const successData = { "data": a_success?.[0]?.data ?? null, "requestid": a_success?.[0]?.requestid ?? null }
...
export { router as fVCBillTranMultiRouter };