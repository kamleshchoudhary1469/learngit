...
router.post('/get-single-tran-rec',
    validationRequest,
    validateRequestBodyKeys(["requestid", "emp_code", "acc_year", "bill_no", "header_req"]),
    ...
    async (req: Request, res: Response) => {
        ...
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi) {
                ...
                const { bill_no, emp_code, requestid, acc_year, header_req } = req.body;

                const procType = 'get_gtdt_tds_tran_load_salary';
                ...
                var sdata = `{
                    "a_session_seqno": "session_seqno_replace",
                        ...
                        "a_in_parameter_json": [{"a_rowid_seq":"${emp_code ?? ""}","a_deductee_panno":"${bill_no ?? ""}"}]
                    }`
...
export { router as singleIncomeRecRouter };