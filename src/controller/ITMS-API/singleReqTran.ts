...
router.post('/get-ifms-single-req-tran',
    validationRequest,
    validateRequestBodyKeys(["requestid", "emp_code", "req_id", "header_req"]),
    ...
    async (req: Request, res: Response) => {
        ...
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi) {
                ...
                const { requestid, req_id, emp_code, header_req } = req.body;
                const procType = 'get_gtdb_req_history_tran';
...
export { router as singleReqTranDetailsRouter };