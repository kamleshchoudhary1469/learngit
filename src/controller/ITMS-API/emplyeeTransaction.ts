...
router.post('/my-income',
    validationRequest,
    validateRequestBodyKeys(["requestid", "emp_code", "acc_year", "header_req"]),
    ...
    async (req: Request, res: Response) => {
        ...
        if (req?.currentUser !== undefined) {
            if (req?.currentUser?.itmsapi) {
                ...
                const procType = 'get_gtdb_tds_tran_load_emp';

                const filterClause = JSON.stringify([{
                    "filter_col_field": "cust_id",
                    "filter_col_depend_operator": "=",
                    "filter_col_value": `${emp_code ?? ""}`,
                    "filter_col_field_alias": "a."
                }]);
                ...
                return res.status(ok).send({
                    status: "SUCCESS", errdata: errors ? errors : null, [procType]: {
                        detail,
                        header
                    }, requestid: requestid, message: success_msg ? success_msg : error_message
                });
...
export { router as myIncomeRouter };